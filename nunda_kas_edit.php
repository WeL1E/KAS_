...
if (isset($_SESSION['id']) && isset($_SESSION['username'])) { //memeriksa id dan username
    ...
    if ($result) {
        ...
        if (isset($_POST['proses'])) {
            ...
            if (empty($nama) || empty($jumlah) || empty($tanggal)) {
                echo "<script>alert('Semua Kolom Harus Terisi');</script>";
            } else {
                // Lakukan pembaruan data ke database
                $query_update = "UPDATE masuk SET nama='$nama', jumlah='$jumlah', tanggal='$tanggal' WHERE id='$id'";
...
<form action="" method="post">
    <table class="tabelbio">
        ...
        <tr>
            ...
            <th><input type="date" name="tanggal" placeholder="tanggal" value="<?php echo $data['tanggal']; ?>"></th>
        </tr>
        <tr>
